...
class es_cls_widget {
	public static function load_subscription($arr) {
		...
		$es = $es . '<div>';
		$es = $es . '<form class="NewsletterCont es_shortcode_form">';
		...
		$es = $es . '<span class="add-on"><i class="fa fa-envelope-o"></i></span>';
//		$es = $es . '<div class="es_textbox">';
			$es = $es . '<input placeholder="Your@email.com" class="es_textbox_class" name="es_txt_email_pg" id="es_txt_email_pg" onkeypress="if(event.keyCode==13) es_submit_pages('.$url.')" value="" maxlength="225" type="text">';
//		$es = $es . '</div>';
//		$es = $es . '<div class="es_button">';
			$es = $es . '<input class="btn btn-large btn-info" onClick="return es_submit_pages('.$url.')" value="'.__('Subscribe', 'email-subscribers').'" type="submit">';
//		$es = $es . '</div>';
		if( $es_name != "YES" ) {
